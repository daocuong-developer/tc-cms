...
import {
  FileText, Users, CheckCircle, BarChart3, Search, Settings, ArrowRight, Menu, Home, Shield, Database, Activity, List
} from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ collapsed, setCollapsed, setUserMenuOpen, activeSection, setActiveSection }) => (
  <div className={`bg-white shadow-lg transition-all duration-300 ${collapsed ? 'w-16' : 'w-64'} flex flex-col fixed h-full z-10`}>
        ...
        {sections.map((section) => {
          ...
          return (
            ...
            >
              ...
              {!collapsed && (
                <span className="ml-3 text-left truncate">{section.title}</span>
              )}
            </button>
          );
        })}
